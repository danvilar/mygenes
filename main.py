...
env_set = str(argv[10])  # Environment set name (must be *.env)
...
params = open('%s/params.txt' % run_name,'w')
...
replicas\t%s
env_set\t\t%s
select_fit\t\t%s''' %(n_cells, n_generations, prob_duplic, prob_elim, prob_delta, prob_alpha, it_steps, replicas,
                      env_set, select_fit))
params.close()
...
class new_cell:
    ...
    def __init__(self, n_counter_newnodes_newedges):
        (n_counter, newnodes, newedges) = n_counter_newnodes_newedges
    ...
    def mutation(self):
        ...
        for node in self.G[0]:
            ...
            for n in copy_nodes:
                if np.random.random_sample() <= prob_delta and n != node and \
                        ((n,node) not in self.G[1] or (node,n) not in self.G[1]):
        ...
        self.G[1] = [(x,y) for (x,y) in new_graph if (x,y) not in self.G[1] and
                     (y,x) not in self.G[1]] + [(x,y) for (x,y) in self.G[1] if (x,y) not in new_graph and (y,x) not in new_graph]
        ...
        for envx in environment: # For each environment...
            ...
            for step in range(it_steps):  # For each step defined above
                for q in boolean_nodes:  # For each node, if it's set to True and it has not been spread yet
                    if q not in spread_nodes and boolean_nodes[q] is True:
            ...
            fit_total *= (fit_parcial**(8./float(len(environment)))) # correction to 8 environments

        # Para evitar erros de NaN
        if len(factive_edges) == 0:
            edge_mod = 0.0
        else:
            edge_mod = float(len([kk for kk,nn in Counter(factive_edges).items()
                                  if nn == 1]))/float(len(set(factive_edges)))

        if len(factive_nodes) == 0:
            node_mod = 0.0
        else:
            node_mod = float(len([kk for kk,nn in Counter(factive_nodes).items()
                                  if nn == 1]))/float(len(set(factive_nodes)))
        #
...
for n_replica in range(replicas):
    # Cell's Birth
    population = [new_cell((0,['N0'],[])) for i in range(n_cells)] # list of the different objects/cells

    print('%s cells were born in replica %s! Congratulations!\n' %(n_cells,int(n_replica+1)))

    # Report Start
    report = open('%s/Report%s.txt' %(run_name,int(n_replica+1)), 'w')
    print('Generation#\tFit_mean\tNode_mod_mean\tEdges_mod_mean\tNodes_mean\tEdges_mean\tFit_std\tNode_mod_std\tEdges_mod_std\tNodes_std\tEdges_std', report)
    ...
    for ii in range(n_generations):
        ...
        fits,nodes_modularity,edges_modularity, nodes_total,edges_total = list(zip(*[x.all_operations()
                                                                                     for x in population]))

        report.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' %(int(ii)+1,np.mean(fits),np.mean(nodes_modularity),
                                                                      np.mean(edges_modularity),np.mean(nodes_total),
                                                                      np.mean(edges_total),np.std(fits),
                                                                      np.std(nodes_modularity),np.std(edges_modularity),
                                                                      np.std(nodes_total),np.std(edges_total)))

        fit_sum = sum(fits)
        new_gen = []

        if fit_sum == 0 or select_fit == 0:
            new_gen = [new_cell(random.choice(population).getAtrib()) for n in range(len(population))]
    ...
    print("End of replica %s in %s total seconds." %(int(n_replica+1), time.time() - start_time)) # simulation time

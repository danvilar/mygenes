...
class Application(Frame):
	...
	def onRun(self):
		query = 'python3 main.py %s %s %s %s %s %s %s %s %s %s 1' %(self.nameEntry.get(), self.ncellsEntry.get(), self.ngenerationsEntry.get(), self.probduplicEntry.get(), self.probelimEntry.get(), self.probdeltaEntry.get(), self.probalphaEntry.get(), self.itstepsEntry.get(), self.replicasEntry.get(), self.envEntry.get())
		system(query)
